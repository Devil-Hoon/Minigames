...
public class CardMemoryGame_Stage3Manager : MonoBehaviour
{
	...
	[Header("Stage Level")]
	public GameObject[] stage3Levels;

	[Header("Round Image")]
	public GameObject stageClear;
	public GameObject stageFail;
	public GameObject[] stage3Rounds;
					...
					if (GameEnd())
					{
						if (!gameAllClear)
						{
							...
							else
							{
								...
								{
									waitTime = 0.0f;
									if (level != LEVEL_THREE.FIVE)
									{
										SetLevel();
										GameInit(level);
									}
									else if (level == LEVEL_THREE.FIVE)
									{
										gameAllClear = true;
									}
								}
						...
						else
						{
							BGSoundManager.instance.StopBGM();
							EffectSoundManager.instance.Stop();
							DBManager.stageNum = StageNum.FOUR;
	...
	{
		for (int i = 0; i < stage3Rounds.Length; i++)
		{
			stage3Rounds[i].SetActive(false);
		}
	}
	...
	public void SetLevel()
	{
		switch (level)
		{
			case LEVEL_THREE.ONE:
				level = LEVEL_THREE.TWO;
				break;
			case LEVEL_THREE.TWO:
				level = LEVEL_THREE.THREE;
				break;
			case LEVEL_THREE.THREE:
				level = LEVEL_THREE.FOUR;
				break;
			case LEVEL_THREE.FOUR:
				level = LEVEL_THREE.FIVE;
				break;
			case LEVEL_THREE.FIVE:
				break;
			default:
				break;
		}
	...
	public void GameInit(LEVEL_THREE level)
	{
		tempTimeCheck = false;
		checkTime = false;
		...
		clearEffect = false;
		BGSoundManager.instance.StopBGM();
		BGSoundManager.instance.PlayCardMemoryGameStage3BGM();
		...
		{
			case LEVEL_THREE.ONE:
				stage3Rounds[0].SetActive(true);
				GetPoses(stage3Levels[0]);
				break;
			case LEVEL_THREE.TWO:
				stage3Rounds[1].SetActive(true);
				GetPoses(stage3Levels[1]);
				break;
			case LEVEL_THREE.THREE:
				stage3Rounds[2].SetActive(true);
				GetPoses(stage3Levels[2]);
				break;
			case LEVEL_THREE.FOUR:
				stage3Rounds[3].SetActive(true);
				GetPoses(stage3Levels[3]);
				break;
			case LEVEL_THREE.FIVE:
				stage3Rounds[4].SetActive(true);
				GetPoses(stage3Levels[4]);
				break;
			default:
				break;
		}
